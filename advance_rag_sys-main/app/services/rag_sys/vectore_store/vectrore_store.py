...
class Vectore_store():
    ...
    def cross_encoder(self , query:str, m : int=3):
        
        #retrieving docs 
        retrieved_docs=self.get_document(query)
        docs= retrieved_docs.get_document(docs,[[]])[0]

        if not docs : 
         return []
        
        #creating pairs for the cross_encoder
        ...
        #ranking and returning the m=3 docs  

        ranked_results= [
        {'docs':doc,'scores':float(score)}
        for (doc,score)in zip (docs,scores)
        ]
        ranked_results.sort(key=lambda x: x["score"], reverse=True)

        return ranked_results[:m]
